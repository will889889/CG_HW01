...
void initCubemapShader()
{
	...
	ShaderInfo shaders[] = {
		...
		{ GL_NONE, NULL } };
	cubemapProgram = LoadShaders(shaders);//讀取shader

	glUseProgram(cubemapProgram);//uniform參數數值前必須先use shader
...
void initZawarudo()
{
	...
	ShaderInfo shaders_zawarudo[] = {
		...
		{ GL_NONE, NULL } };
	program_zawarudo = LoadShaders(shaders_zawarudo);//讀取shader

	glUseProgram(program_zawarudo);//uniform參數數值前必須先use shader
